...
public class WavProcessing : MonoBehaviour
{
    ...
    {
        // Changed from .mp3 to .wav (server now generates WAV files)
        string therapistWavPath = Path.Combine(Application.persistentDataPath, $"{BaseWavPath}therapist_speech.wav");

        // Use Flask endpoint to serve the audio file (handles spaces in path correctly)
        string encodedPath = UnityWebRequest.EscapeURL(therapistWavPath);
        string audioUrl = $"{baseUri}/get_audio/{encodedPath}";

        Debug.Log($"Loading audio from: {audioUrl}");

        // Load the audio clip from Flask server
        using (UnityWebRequest www = UnityWebRequestMultimedia.GetAudioClip(audioUrl, AudioType.WAV))
        {
            ...
            switch (www.result)
            {
                ...
                case UnityWebRequest.Result.ProtocolError:
                    ...
                    Debug.LogError($"Tried to load from: {audioUrl}");
                    break;
                case UnityWebRequest.Result.Success:
                    ...
                    {
                        Debug.Log($"Audio clip loaded successfully. Length: {audioSource.clip.length}s");
                    }
